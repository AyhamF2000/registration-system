...
async def handle_google_auth(callback=False):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if email:
            ...
            if is_new_user:
                welcome_message = await call_node_server(f"Welcome {name} to Elysian Softech!(https://elysian-softech.com/)")
                return redirect(f"{Config.FRONTEND_URL}/welcome?name={name}&email={email}&message={message}&welcome_message={welcome_message}")
            else:
                welcome_message = await call_node_server(f"Welcome back, {name}! Here’s a random programming fact.")
                return redirect(f"{Config.FRONTEND_URL}/welcome?name={name}&email={email}&message={message}&welcome_message={welcome_message}")
